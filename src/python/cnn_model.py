...
class Autoencoder():
    ...
    def __init__(self, dataset, dims, epochs, batch_size, latent_dim):
    ...
    def process_data(self, testSize, randomState=13):
    ...
    def encoder(self, input_img):
        ...
        conv = layers.Conv2D(128, kernel_size=3, padding='same', strides=(2, 2), activation='relu')(conv)
        
        self.shape_before_flattening = K.int_shape(conv)

        conv = layers.Flatten()(conv)
        conv = layers.Dense(32, activation='relu')(conv)

        self.z_mu = layers.Dense(self.latent_dim)(conv)
        self.z_log_sigma = layers.Dense(latent_dim)(conv)
    ...
    def sampling(self):
        epsilon = K.random_normal(shape=(K.shape(self.z_mu)[0], self.latent_dim), mean=0, stddev=1.)
        return self.z_mu + K.exp(z_log_sigma) * epsilon
    ...
    def decoder(self, decoder_input):
        ...
        conv = layers.Conv2DTranspose(128, kernel_size=3, padding='same', strides=(2, 2), activation='relu')(conv)
        ...
        conv = layers.Conv2D(1, kernel_size=3, padding='same', activation='sigmoid')(conv)

        return keras.Model(decoder_input, conv)
        # conv = None
